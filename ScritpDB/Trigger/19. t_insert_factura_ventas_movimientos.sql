-- FUNCTION: public.t_insert_factura_ventas_movimientos()

-- DROP FUNCTION IF EXISTS public.t_insert_factura_ventas_movimientos();

CREATE OR REPLACE FUNCTION public.t_insert_factura_ventas_movimientos()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION public.t_insert_factura_ventas_movimientos()
    OWNER TO "MiMascota";


-- Trigger: t_insert_factura_ventas_movimientos

-- DROP TRIGGER IF EXISTS t_insert_factura_ventas_movimientos ON public.factura_ventas;

CREATE OR REPLACE TRIGGER t_insert_factura_ventas_movimientos
    AFTER INSERT
    ON public.factura_ventas
    FOR EACH ROW
    EXECUTE FUNCTION public.t_insert_factura_ventas_movimientos();
